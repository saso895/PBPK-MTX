...
param = pars
#-------------参数对数化------------------------
log_param0  = np.log(pars)                           # 初始点
log_bounds  = [(np.log(0.01*p), np.log(10.0*p))      # 边界：0.01× ~ 10×
               for p in pars]
call_count = 0
...
def total_cost_logtheta(logtheta, time_pts, conc_data):
    """包装后的目标函数：接收 logθ，内部转回物理量"""
    theta = np.exp(logtheta)
    return total_cost(theta, time_pts, conc_data)
...
#def total_cost(params, time_points_train, concentration_data_train):
result = minimize(total_cost_logtheta, 
                  log_param0,
                  args = (time_points_train, concentration_data_train),
                  method = 'Powell',
                  bounds=log_bounds,
                  options={'disp': True, 'adaptive': True, 'maxiter': 5000}
                  )# , options=optionsbounds=bounds,
...
# 保存优化后的参数
with open('saved_result\optimized_params0528_Powell.pkl', 'wb') as f:
    pickle.dump(popt, f)

print("优化参数已保存")

# ───── 参数对比表 ─────
import pandas as pd             # 文件顶部已 import pandas 就跳过

param_names = ['PRest', 'PK', 'PL', 'Kbile', 'GFR',
               'Free', 'Vmax_baso', 'Km_baso', 'Kurine', 'Kreab']

init_vals = [init_pars[n] for n in param_names]   # 初始参数
fit_vals  = popt                                  # 拟合参数 (线性尺度)
...
# 可选：保存到 Excel 便于后期查阅
df_param.to_excel('saved_result/params_compare_0528.xlsx', index=False)
print("参数对比表已写入 saved_result/params_compare_0528.xlsx")