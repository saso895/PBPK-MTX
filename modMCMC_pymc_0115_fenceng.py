...
if __name__ == '__main__':
    ...
    n_states=7
    n_theta=10
    n_groups = len(time_points_train)
    ...
    with pm.Model() as model:
        ...
        PRest_group = pm.Normal('PRest_group', mu=PRest, sigma=1, shape=n_groups)
        PK_group = pm.Normal('PK_group', mu=PK, sigma=1, shape=n_groups)
        PL_group = pm.Normal('PL_group', mu=PL, sigma=1, shape=n_groups)
        Kbile_group = pm.Normal('Kbile_group', mu=Kbile, sigma=1, shape=n_groups)
        GFR_group = pm.Normal('GFR_group', mu=GFR, sigma=1, shape=n_groups)
        Free_group = pm.Normal('Free_group', mu=Free, sigma=1, shape=n_groups)
        Vmax_baso_group = pm.Normal('Vmax_baso_group', mu=Vmax_baso, sigma=1, shape=n_groups)
        Km_baso_group = pm.Normal('Km_baso_group', mu=Km_baso, sigma=1, shape=n_groups)
        Kurine_group = pm.Normal('Kurine_group', mu=Kurine, sigma=1, shape=n_groups)
        Kreab_group = pm.Normal('Kreab_group', mu=Kreab, sigma=1, shape=n_groups)
        ...
        for i in range(n_groups):
            ...
            params = [PRest_group[i], PK_group[i], PL_group[i], Kbile_group[i], GFR_group[i],\
                   Free_group[i], Vmax_baso_group[i], Km_baso_group[i], Kurine_group[i], Kreab_group[i]]
            ...
            y_obs = pm.Normal(f'y_obs_{i}', mu=y[:, 0] / VPlas, sigma=0.1, observed=concentration)
