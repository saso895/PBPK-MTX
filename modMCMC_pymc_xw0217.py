...
def derivshiv(y, t, params):
    ...
    return [ydot0 / VPlas,ydot1 / VL,ydot2 / VK,ydot3 / VRest,ydot4,ydot5,ydot6]#
...
if __name__ == '__main__':
    ...
    with pm.Model() as model:
        ...
        Vmax_baso = pm.Lognormal('Vmax_baso', mu=pm.math.log(init_pars["Vmax_baso"]), sigma=10)
        ...
        for i in tqdm(range(len(time_points_train))):
            ...
            concentration = np.random.lognormal(mean=concentration_data_train[i], sigma=0.2)
            ...
            trace = pm.sample(100, chains=4, tune=30, target_accept=0.9, cores=28, random_seed=random_seed)
            ...
            PRest = pm.Lognormal('PRest', mu=pm.math.log(summary.loc['PRest', 'mean']), sigma=1)
            PK = pm.Lognormal('PK', mu=pm.math.log(summary.loc['PK', 'mean']), sigma=1)
            PL = pm.Lognormal('PL', mu=pm.math.log(summary.loc['PL', 'mean']), sigma=1)
            Kbile = pm.Lognormal('Kbile', mu=pm.math.log(summary.loc['Kbile', 'mean']), sigma=1)
            GFR = pm.Lognormal('GFR', mu=pm.math.log(summary.loc['GFR', 'mean']), sigma=1)
            Free = pm.Lognormal('Free', mu=pm.math.log(summary.loc['Free', 'mean']), sigma=0.1)
            Vmax_baso = pm.Lognormal('Vmax_baso', mu=pm.math.log(summary.loc['Vmax_baso', 'mean']), sigma=10)
            Km_baso = pm.Lognormal('Km_baso', mu=pm.math.log(summary.loc['Km_baso', 'mean']), sigma=1)
            Kurine = pm.Lognormal('Kurine', mu=pm.math.log(summary.loc['Kurine', 'mean']), sigma=0.01)
            Kreab = pm.Lognormal('Kreab', mu=pm.math.log(summary.loc['Kreab', 'mean']), sigma=0.01)
