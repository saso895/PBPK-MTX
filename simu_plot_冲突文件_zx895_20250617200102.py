...
#==========读入模拟数据
with open('saved_result\modfitPowell_0427.pkl', 'rb') as f:
    y_GA=pickle.load( f)
with open('saved_result/modfitPowell_0613.pkl', 'rb') as f:
    y_FIT=pickle.load( f)
with open('ini_params_simu0427.pkl', 'rb') as f:
    y_ini=pickle.load( f)
with open('fitGA_params_simu0427.pkl', 'rb') as f:
...
with tqdm(range(len(time_points_train))) as pbar:
    ...
    for i in pbar:
        ...
        axes[i].plot(FIT_y[:,0], FIT_y[:,1], label=f'0613fit拟合曲线 组 {i+1}', color='#5ca788')
        axes[i].plot(GA_y[:,0], GA_y[:,1], label=f'0427fit曲线 组 {i+1}', color='#fdd363',lw=4)
...
# 保存图像为矢量图格式
save_path =f'saved_result/Simuplot_Powell0615.svg'
