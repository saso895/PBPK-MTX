...
#==========读入模拟数据
with open(f'{server}result\chain1_0620_{server}.pkl', 'rb') as f:
    y_chain1=pickle.load( f)
with open(f'{server}result/chain2_0620_{server}.pkl', 'rb') as f:
    y_chain2=pickle.load( f)
with open(f'{server}result/chain3_0620_{server}.pkl', 'rb') as f:
    y_chain3=pickle.load( f)
with open(f'{server}result\chain4_0620_{server}.pkl', 'rb') as f:
    y_chain4=pickle.load( f)      
with open(f'{server}result/chain6_0620_{server}.pkl','rb') as f:
...
# # === 保存所有链的拟合图 ==================================================
# save_path =f'{server}result/Simuplot_{today_date}_95.svg'
...
# === 保存拟合图 ==================================================
save_path =f'{server}result/Simuplot_chain1{today_date}_95.svg'
plt.savefig(save_path, format='svg')
plt.show()

# === 🔵 结果输出为 Excel 表格 ====================================
df_result = pd.DataFrame(result_rows, columns=["Patient_ID", "AFE", "AAFE",  "CP90","Tag"])#
excel_path = f'{server}result/Patient_Errors_{today_date}.xlsx'
...
plt.tight_layout()
heatmap_path = f'{server}result/Heatmap_{today_date}.png'
plt.savefig(heatmap_path, dpi=300)
plt.close()
print(f"✅ 热图已保存: {heatmap_path}")

# ③   保存 good 病人 ID 清单                # === NEW ===
good_id_path = f'{server}result/good_patient_ids_{today_date}.txt'
