...
if __name__ == '__main__':
    ...
    #         weights = np.where(time_points <= 24, 1.0, 0.6)
    #         weighted_sigma = sigma / weights
    ...
    #     np.save(os.path.join('saved_result/', 'best_params0408.npy'), best_params)
     
    ############---------MCMC优化之后的浓度预测------------#################
    with tqdm(range(len(time_points_train))) as ybar:
        best_params = np.load('saved_result/best_params0408.npy')
        y_pred = []
        # 绘制观测值和后验预测值
