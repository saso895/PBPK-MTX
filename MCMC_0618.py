...
def run_chain(seed,chain_id=None, progress_bar=True):
    ...
    curr_log   = theta_log_start + rng.normal(scale=0.10)# ★★ 修改 ：链起点加 10 % 抖动
    #curr_theta = theta_start.copy()
    curr_theta = np.exp(curr_log)#★★ 修改 
    ...
    #    iters = tqdm(iters, desc=desc_txt, leave=False)
...
if __name__ == "__main__":
    ...
    for cid in trange(n_chain, desc="Sequential sampling"):
        print(f"\n—— 链 {cid+1}/{n_chain} 采样开始 ——")
        chain_c, ll_c, acc_c = run_chain(seed=20240611 + cid, chain_id=cid + 1)
        chain_list.append(chain_c)
        loglike_list.append(ll_c)
        acc_rates.append(acc_c)
        print(f"链 {cid+1} 完成，接受率 {acc_c:.2f}")
    ...
    az.plot_trace(idata)
    plt.tight_layout()
    plt.savefig("/nfs/home/y18300744/MTXmodel/saved_result/mcmc_traceplot0612.png", dpi=300)
    plt.close()
    print("\n=== ArviZ 收敛诊断 ===")
    ...
    out_path = '/nfs/home/y18300744/MTXmodel/saved_result/mcmc_params0612.pkl'
