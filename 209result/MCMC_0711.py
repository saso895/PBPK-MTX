...
with open('/nfs/home/y18300744/MTXmodel/saved_result/modfit_params2025-07-11.pkl', 'rb') as f:
...
# =============== 5. 采样参数
checkpoint_int = 2#20000          # ★★ 新：每 1万 步存一次 ckpt
resume_path    = ""              # ★★ 新：空字串=不续跑；填路径即续
jit_scale = 0.03        # <-- 更小初始抖动幅度 (log 空间±5 %)
step_report = 4#40000
n_iter   = 12#120000      # 总迭代
burn_in  = 2#20000       # 丢弃前 burn_in
thin     = 1#100          # 每 thin 取一次，减少自相关
n_chain = 4
rng = np.random.default_rng(seed=20240711)
...
if __name__ == "__main__":
    results = Parallel(n_jobs=int(n_chain), prefer="processes")(    # <<< PATCH ④
        delayed(run_chain)(seed=20240611 + cid, chain_id=cid+1)
        for cid in range(int(n_chain))
    )
