...
import arviz as az
from ode_core import derivshiv,FIT_model,log_normalize,exp_denormalize,PK_model
...
# === 2. 读入 Powell 先验中心 ===============================
with open('/nfs/home/y18300744/MTXmodel/saved_result/optimized_params2025-07-07.pkl', 'rb') as f:
...
# =============== 5. 采样参数
checkpoint_int = 50_000          # ★★ 新：每 1万 步存一次 ckpt
resume_path    = ""              # ★★ 新：空字串=不续跑；填路径即续
jit_scale = 0.05        # <-- 初始抖动幅度 (log 空间±5 %)
step_report = 40_000
n_iter   = 200_000      # 总迭代
burn_in  = 20_000       # 丢弃前 burn_in
thin     = 50          # 每 thin 取一次，减少自相关
#rng = np.random.default_rng(seed=20240707)
n_chain = 4
#rng_seeds = [20240630 + i for i in range(n_chain)]  ### >>> NEW
# === 6. 单链采样函数 ==============================================
def run_chain(seed,chain_id=None, progress_bar=True):
    ...
    for i in iters:
        ...
        if (i + 1) % checkpoint_int == 0:
            ck_path = f"saved_result/ckpt_chain{chain_id}_{i+1}.pkl"
            pickle.dump({
                "iter":   i + 1,
                "phi":    curr_log,
                "theta":  curr_theta,
...
if __name__ == "__main__":
    ...
    for k, pc in enumerate(post_list, 1):          # post_list 里是 burn-in 后样本
        ...
        draw_path = f"/nfs/home/y18300744/MTXmodel/saved_result/chain{k}_draws{today_date}.pkl"
        ...
        path_k  = f"/nfs/home/y18300744/MTXmodel/saved_result/chain{k}_params{today_date}.pkl"
...
for k in range(step_report, n_iter + 1, step_report):
    ...
    out_xlsx = f"/nfs/home/y18300744/MTXmodel/saved_result/mcmc_diag_{today_date}.xlsx"
    rep_df.to_excel(out_xlsx, index=False)
    print(f"\n✅ 诊断表已保存 ➜ {out_xlsx}")
    # ------------------------------------------------------------
    plt.tight_layout()
    plt.savefig(f"/nfs/home/y18300744/MTXmodel/saved_result/mcmc_traceplot{today_date}.png", dpi=300)
    plt.close()

    # === 8. 保存后验均值 ============
    #os.makedirs('saved_result', exist_ok=True)
    out_path = f'/nfs/home/y18300744/MTXmodel/saved_result/mcmc_params{today_date}.pkl'
    ...
    # === 10. 采样收敛诊断 (R-hat / ESS) ============================  ←←【新增 ②】
    try:
        import arviz as az
        import numpy as np, pandas as pd, os
        data_np = np.stack(post_list)                       # (chain, draw, par)
        posterior = {name: data_np[:,:,i]                   # 转 ArviZ InferenceData
                     for i, name in enumerate(param_names)}
        idata = az.from_dict(posterior=posterior)

        diag = az.summary(idata, var_names=param_names)[
                   ["r_hat","ess_bulk","ess_tail"]]
        diag_csv = f"/nfs/home/y18300744/MTXmodel/saved_result/rhat_ess_{today_date}.csv"
