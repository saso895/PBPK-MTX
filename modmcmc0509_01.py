...
def derivshiv(y, t, theta, R, T_total):
    ...
    # Python 布尔改用 tt.switch 以符号化
    input_rate = tt.switch(tt.le(t, T_total), R, 0)

    dot0 = (QRest * y[3] / VRest / PRest) \
         + (QK   * y[2] / VK   / PK) \
         + (QL   * y[1] / VL   / PL) \
         - (QPlas* y[0] / VPlas) \
         + Kreab * y[4] \
    ...
    vec = tt.reshape(vec, (7,))                 # rank-1
    return tt.specify_shape(vec, (7,)) 
    #return tt.stack([dot0, dot1, dot2, dot3, dot4, dot5, dot6]).flatten()
