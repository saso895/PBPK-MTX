...
with tqdm(range(len(time_points_train))) as ybar:
    ...
    with open('saved_result/SA_param2025-07-02.pkl', 'rb') as f:#saved_result/
        saved = pickle.load(f)              # 读回 dict
    if isinstance(saved, dict):
        baseline = np.asarray(saved['baseline'])   # 只取拟合好的 10 维参数
    else:
        baseline = np.asarray(saved)[:10]          # 老文件格式, 前 10 项就是参数
    #    saved = saved[:10] 
    #baseline = saved 
...
save_path =f'saved_result/SA_simu{today_date}.pkl' 
