...
result = minimize(cost_function, initial_guess_normalized, args=(time_points_train, concentration_data_train,input_dose_train,inject_timelen_train), method='L-BFGS-B')
...
print(f"参数优化运行时间: {elapsed_time:.2f} 秒")

with open('pars/modfit_pars.pkl', 'wb') as f:
    pickle.dump(popt, f)
...
for i in range(len(time_points_train)):
    ...
    pred_y = pk_model(time, D_total, T_total, *popt)
