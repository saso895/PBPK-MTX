...
for outer in range(max_outer):
    ...
    cands = GammaDF[GammaDF["Gamma"] < gamma_thresh]

    if cands.empty:
        # 没有任何子集 γ<阈值 —— 降低阈值或保留最小 γ 的子集
        best_row = GammaDF.loc[GammaDF["Gamma"].idxmin()]
        print(f"⚠️ 没有 γ<{gamma_thresh} 的子集，暂取 γ 最小组合")
    else:
        # 1) 取最大 k
        k_max = cands["Subset size"].max()
        cands_k = cands[cands["Subset size"] == k_max]
        # 2) 在 k_max 里取 γ 最小
        best_row = cands_k.loc[cands_k["Gamma"].idxmin()]
