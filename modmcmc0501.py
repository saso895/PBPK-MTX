...
if __name__ == '__main__':
    ...
    with pm.Model() as model:
        ...
        step=pm.Metropolis()
        trace = pm.sample(1000, step=step,start=start,chains=2, tune=400,  cores=4, discard_tuned_samples=True, random_seed=1)#target_accept=0.9,
        ...
        with open('saved_result/best_params0501.pkl', 'wb') as f:
            pickle.dump(best_params, f)
        print("MCMC优化参数已保存")
     
    ############---------MCMC优化之后的浓度预测------------#################
    with tqdm(range(len(time_points_train))) as ybar:
        with open('saved_result/best_params0501.pkl', 'rb') as f:
    ...
    with open('saved_result\y_MCMC0501.pkl', 'wb') as f:
