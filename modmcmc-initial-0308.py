...
if __name__ == '__main__':
    ...
    #     Vmax_baso = pm.Lognormal('Vmax_baso', mu=pm.math.log(fit_params[6]), sigma=10)
    ...
    #     step=pm.NUTS()
    #     trace = pm.sample(1000, step=step,start=start,chains=4, tune=500,  cores=20, discard_tuned_samples=False, random_seed=1)#target_accept=0.9,
    ...
    #     np.save(os.path.join('saved_result/', 'best_params0308_1.npy'), best_params)
     
    ############---------MCMC优化之后的浓度预测------------#################
    with tqdm(range(len(time_points_train))) as ybar:
        best_params = np.load('saved_result/best_params0308_1.npy')
        y_pred = []
        # 绘制观测值和后验预测值
        # pbar = tqdm(range(len(time_points_train)))
        for i in ybar:
    ...
    file_path = os.path.join(save_dir, 'y_MCMC0308_1.pkl')
