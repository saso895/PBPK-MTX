...
for outer in range(max_outer):
    ...
    for idx, (pname, theta0) in enumerate(zip(param_names, baseline)):
        ...
        #up[idx]   += dtheta
        #down[idx] -= dtheta
        dtheta = delta_rel                 # ln(θ) 轴上的 ±0.05
        up[idx]   *= math.exp( delta_rel)  # θ × 1.05127...
        down[idx] *= math.exp(-delta_rel)  # θ × 0.95123...
        #
        for  t, d, tinf in zip(time_groups, dose_groups, tinf_groups):
            diff_seg = (FIT_model(t, d, tinf, *up) - 
                        FIT_model(t, d, tinf, *down)) / (2 * dtheta)
            diff_parts_norm.append(diff_seg / sy_global)   # 全局归一化


        sl = np.concatenate(diff_parts_norm)    # 公式(4)
