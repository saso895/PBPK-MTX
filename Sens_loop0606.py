...
for outer in range(max_outer):
    ...
    sy = np.sqrt(np.mean((y_obs - y_pred)**2))# 求出sy--归一化向量

    for idx, (pname, theta0) in enumerate(zip(param_names, baseline)):
        dtheta = theta0 * delta_rel if theta0 != 0 else 1e-6               #theta0 * delta_rel，构建扰动，theta0是一个base，0.028*base
    ...
    # 按子集大小分工作表写入
    with pd.ExcelWriter("gamma_subsets0606.xlsx", engine="openpyxl") as writer:
