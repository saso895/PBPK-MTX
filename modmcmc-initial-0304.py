...
if __name__ == '__main__':
    ...
    with pm.Model() as model:
        ...
        np.save(os.path.join('saved_result/', 'best_params0306.npy'), best_params)
     
    ############---------MCMC优化之后的浓度预测------------#################
    with tqdm(range(len(time_points_train))) as ybar:
        best_params = np.load('saved_result/best_params0306.npy')
        y_pred = []
        # 绘制观测值和后验预测值
        # pbar = tqdm(range(len(time_points_train)))
        for i in ybar:
    ...
    file_path = os.path.join(save_dir, 'y_MCMC0305.pkl')
