...
for outer in range(max_outer):
    ...
    base_curve, weights = [], []
    # ── 2.1 先计算每个病人的 sy_scaled ──────────────────────────────
    sy_scaled_list = []
    for t, d, tinf, obs in zip(time_groups, dose_groups, tinf_groups, conc_groups):
        y_pred_i = FIT_model(t, d, tinf, *baseline)
        sy_i = np.sqrt(np.mean((obs - y_pred_i) ** 2))
        sy_i = max(sy_i, 1e-12)                    # 避免 0
        sy_scaled_i = sy_i / np.max(y_pred_i)
        sy_scaled_list.append(sy_scaled_i)

    # ── 2.2 逐参数扰动 → 计算灵敏度向量 sl_j ───────────────────────
    for idx, (pname, theta0) in enumerate(zip(param_names, baseline)):
        ...
        # 针对每位病人独立计算差分 & 归一化
        diff_parts_norm = []
        for (t, d, tinf), sy_sc in zip(zip(time_groups, dose_groups, tinf_groups),
                                    sy_scaled_list):
            diff_seg = (FIT_model(t, d, tinf, *up) -
                        FIT_model(t, d, tinf, *down)) / (2 * dtheta)
            diff_parts_norm.append(diff_seg / sy_sc)     # 归一化

        sl = dtheta * np.concatenate(diff_parts_norm)    # 公式(4)
        sl_vectors.append(sl)                            # 公式(5)
        S_global.append(np.sqrt(np.mean(sl ** 2)))       # 公式(6)    
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("gamma_verticals0603.png", dpi=300)
    plt.show()
    ...
    # 按子集大小分工作表写入
    with pd.ExcelWriter("gamma_subsets0603.xlsx", engine="openpyxl") as writer:
