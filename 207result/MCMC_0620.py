...
with open('/nfs/home/h18300744/MTXmodel/saved_result/optimized_params0427_Powell.pkl', 'rb') as f:
...
# --- ❶ θ_ref 作为缩放基准（可用文献或 theta_start）-------------
theta_ref = theta_start.copy()        # 这里直接用原 Powell 初值
phi0      = np.log10(theta_start / theta_ref)   # 采样初始点 (11,)
...
phi_start = phi0  
step_sizes = 0.05 * np.ones_like(phi_start)    # φ 空间 5 % 抖动
...
n_iter   = 50000       # 总迭代
burn_in  = 10000       # 丢弃前 burn_in
thin     = 1          # 每 thin 取一次，减少自相关
...
def run_chain(seed,chain_id=None, progress_bar=True):
    ...
    curr_phi   = phi_start + rng.normal(scale=jit_scale)#★★ 修改 
    curr_theta = theta_ref * 10**curr_phi
    ...
    for i in iters:
        ...
        prop_phi   = propose(curr_phi, rng)
        prop_theta = theta_ref * 10**prop_phi
        prop_ll    = log_likelihood(prop_theta)

        if np.log(rng.random()) < (prop_ll - curr_ll):#★★ 修改 
            curr_phi, curr_theta, curr_ll = prop_phi, prop_theta, prop_ll
            accept_cnt += 1
...
if __name__ == "__main__":
    ...
    import pickle, os
    os.makedirs('/nfs/home/h18300744/MTXmodel/saved_result', exist_ok=True)
    ...
    for k, pc in enumerate(post_list, 1):          # post_list 里是 burn-in 后样本
        ...
        path_k  = f"/nfs/home/h18300744/MTXmodel/saved_result/chain{k}_params.pkl"
    ...
    az.plot_trace(idata)
    plt.tight_layout()
    plt.savefig("/nfs/home/h18300744/MTXmodel/saved_result/mcmc_traceplot0620.png", dpi=300)
    plt.close()

    # === 8. 保存后验均值 ============
    #os.makedirs('saved_result', exist_ok=True)
    out_path = '/nfs/home/h18300744/MTXmodel/saved_result/mcmc_params0620.pkl'
