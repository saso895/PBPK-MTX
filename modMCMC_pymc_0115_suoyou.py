...
def generate_dataset():
    ...
    for i in range(len(time_points_train)):
        # prefix = '1'+'0'*2+'1'+ str(i)
        for _ in time_points_train[i]:
            ids.append(id)
            id += 1
            # postfix = str(time)
            # ctime = float(prefix+postfix)
            # times.append(ctime)
        y_obs.append(concentration_data_train[i])
        inject_timelens.append(inject_timelen_train[i])
        R = input_dose_train[i] / inject_timelen_train[i]
        inject_rates.append(R)
    return np.concatenate(times), np.concatenate(y_obs), np.array(inject_timelens), np.array(inject_rates), np.array(ids)
...
if __name__ == '__main__':
    ...
    times, y_obs, inject_timelens, inject_rates, ids = generate_dataset()
    ...
    with pm.Model() as model:
        ...
        Y_obs = pm.Normal('Y_obs', mu=y[:, 0]/VPlas, sd=1, observed=y_obs)
